...
void lheparse() {
	...
	TFile f("tree.root","RECREATE");    //creates a .root file to write data to.
...
//	lheparse(); //this is the function that is called by lhe.C, which then proceeds to run the two above functions in order.
	...
	cout << "Job Complete! Time Elapsed = " << double(end-begin)/CLOCKS_PER_SEC/60 << " minutes (" << double(end-begin)/CLOCKS_PER_SEC << " seconds)" << endl; //outputs the time spent in minutes and seconds
}
